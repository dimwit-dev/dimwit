...
class AutodiffSuite extends AnyFunSpec with Matchers:
  ...
  describe("jacRev / jacFwd"):
    ...
    val engines = List(
      ("jacRev", [In: ToFloatTensorTree, Out: ToTensorTree] => (f: In => Out) => (gradTree: ToTensorTree[Gradient[In, Out]]) ?=> Autodiff.jacRev[In, Out](f)),
      ("jacFwd", [In: ToFloatTensorTree, Out: ToTensorTree] => (f: In => Out) => (gradTree: ToTensorTree[Gradient[In, Out]]) ?=> Autodiff.jacFwd[In, Out](f))
    )
