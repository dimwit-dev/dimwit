...
class TensorOpsContractionSuite extends AnyFunSpec with ScalaCheckPropertyChecks with Matchers:

  py.exec("import jax.numpy as jnp")

  describe("contract operation"):
    it("Tensor1[a] and Tensor1[a] to Tensor0"):
    ...
    it("Tensor2[a, b] and Tensor2[a, b] on a"):
    ...
    it("Tensor2[a, b] and Tensor2[a, b] on b"):
      forAll(twoSameTensor2Gen(VType[Float])): (t1, t2) =>
    ...
    it("invariant to Tensor2 axis order"):
      forAll(twoSameTensor2Gen(VType[Float])): (t1, t2) =>
  ...
  describe("outerProduct operation"):
    it("Tensor1[a] and Tensor1[a] to Tensor2[a, a']"):
