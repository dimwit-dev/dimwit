...
def main =
  {
    ...
    val t0_f32_2: Tensor0[Float32] = Tensor0[Float32](0.0)
    val t0_f64_1: Tensor0[Float64] = Tensor0(0.0)
    val t0_f64_2: Tensor0[Float64] = Tensor0[Float64](0f)
  }
  {
    val array = Array(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f)
    val t_i32: Tensor1[A, Int32] = Tensor1(Axis[A]).fromArray(array.map(_.toInt))
    val t_i64: Tensor1[A, Int64] = Tensor1(Axis[A]).fromArray[Int64](array.map(_.toInt))
  ...
  }
  {
    ...
    val t_f32_2: Tensor[(A, B), Float32] = Tensor(shape).fill[Float32](0.0)
    val t_f64_1: Tensor[(A, B), Float64] = Tensor(shape).fill(0.0)
    val t_f64_2: Tensor[(A, B), Float64] = Tensor(shape).fill[Float64](0f)
  }
  {
    val shape = Shape2(Axis[A] -> 2, Axis[B] -> 3)
    val array = Array(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f)
    val t_f32_1: Tensor[(A, B), Float32] = Tensor(shape).fromArray(array)
    val t_f32_2: Tensor[(A, B), Float32] = Tensor(shape).fromArray[Float32](array)
    val t_f64_1: Tensor[(A, B), Float64] = Tensor(shape).fromArray(array.map(_.toDouble))
    val t_f64_2: Tensor[(A, B), Float64] = Tensor(shape).fromArray[Float64](array.map(_.toDouble))
  }
  {
    val shape = Shape2(Axis[A] -> 2, Axis[B] -> 3)
    val array = Array(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f)
